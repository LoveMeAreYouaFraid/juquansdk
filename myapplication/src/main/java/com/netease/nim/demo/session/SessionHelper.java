...
import com.netease.nim.demo.session.action.AudioAndVideoAction;
...
import com.netease.nim.demo.session.action.ITADAction;
...
import com.netease.nim.demo.session.viewholder.MsgViewHolderBringGoods;
...
import com.netease.nim.demo.session.viewholder.MsgViewHolderStickerCollection;
...
//        NimUIKit.setCommonTeamSessionCustomization(getTeamCustomization(null));
    ...
    public static void startP2PSession2(Context context, String account,boolean isService) {
        startP2PSession2(context, account, null,isService);
    }
    ...
    public static void startP2PSession2(Context context, String account, IMMessage anchor,boolean isService) {
        if (!DemoCache.getAccount().equals(account)) {
            if (NimUIKit.getRobotInfoProvider().getRobotByAccount(account) != null) {
                NimUIKit.startChatting2(context, account, SessionTypeEnum.P2P, getRobotCustomization(), anchor,isService);
            } else {
                NimUIKit.startP2PSession2(context, account, anchor,isService);
            }
        } else {
            NimUIKit.startChatting2(context, account, SessionTypeEnum.P2P, getMyP2pCustomization(), anchor,isService);
        }
    }
    ...
    public static void startTeamSession(Context context, String tid, boolean isVip) {
        NimUIKit.startTeamSession(context, tid, getTeamCustomization(tid, isVip), null);
    }

    public static void startTeamSession(Context context, String tid, IMMessage anchor) {
        NIMClient.getService(TeamService.class).searchTeam(tid).setCallback(new RequestCallback<Team>() {
            @Override
            public void onSuccess(Team team) {
                NimUIKit.startTeamSession(context, tid, getTeamCustomization(tid, team.getMemberLimit() >= 2000), anchor);
            }
    ...
    // 打开社群界面(用于 UIKIT 中部分界面跳转回到指定的页面)
    public static void startTeamSession(Context context, String tid, Class<? extends Activity> backToClass,
                                        IMMessage anchor) {
        NIMClient.getService(TeamService.class).searchTeam(tid).setCallback(new RequestCallback<Team>() {
            @Override
            public void onSuccess(Team team) {
                NimUIKit.startChatting(context, tid, SessionTypeEnum.Team, getTeamCustomization(tid, team.getMemberLimit() >= 2000), backToClass, anchor);
            }
    ...
    private static SessionCustomization getP2pCustomization() {
        if (p2pCustomization == null) {
            p2pCustomization = new SessionCustomization() {
                ...
                @Override
                public MsgAttachment createStickerCollectAttachment(String emoji_name, String emoji_id, String emoji_link) {
                    return new StickerCollectAttachment(emoji_name, emoji_id, emoji_link);
                }
            };
            // 背景
...
//            actions.add(new AVChatAction(AVChatType.AUDIO));
            ...
            actions.add(new AudioAndVideoAction());
            actions.add(new RedPacketAction());
            actions.add(new SnapChatAction());//增加阅后即焚
    ...
    private static SessionCustomization getTeamCustomization(String tid, boolean isVip) {
...
//            }
        ...
        if (advancedTeamCustomization == null || advancedTeamCustomization != null) {//因为id会变，必须重建
            // 定制加号点开后可以包含的操作， 默认已经有图片，视频等消息了
            ...
            actions.add(avChatAction);
            if (isVip) {
//                actions.add(new VideoADAction(tid));
                actions.add(new ITADAction(tid));
            }

            actions.add(new FileAction());//群聊发送文件
            actions.add(new SnapChatAction());//群聊增加阅后即焚
            actions.add(new RedPacketAction());
            actions.add(new CollectAction());//收藏
            SessionTeamCustomization.SessionTeamCustomListener listener = new SessionTeamCustomization.SessionTeamCustomListener() {
    ...
    private static void registerViewHolders() {
        ...
        NimUIKit.registerMsgItemViewHolder(LiveAttachment.class, MsgViewHolderLive.class);
        NimUIKit.registerMsgItemViewHolder(ShareProductAttachment.class, MsgViewHolderProduct.class);
        NimUIKit.registerMsgItemViewHolder(CreditAmountAttachment.class, MsgViewHolderCreditTrans.class);
        NimUIKit.registerMsgItemViewHolder(ShareVideoAttachment.class, MsgViewHolderShareVideo.class);
        NimUIKit.registerMsgItemViewHolder(StickerCollectAttachment.class, MsgViewHolderStickerCollection.class);
    }
    ...
    private static void registerRedPacketViewHolder() {
        ...
        NimUIKit.registerMsgItemViewHolder(BringGoodsAttachment.class, MsgViewHolderBringGoods.class);
//        if (NIMRedPacketClient.isEnable()) {
//            NimUIKit.registerMsgItemViewHolder(RedPacketAttachment.class, MsgViewHolderRedPacket.class);
//            NimUIKit.registerMsgItemViewHolder(RedPacketOpenedAttachment.class, MsgViewHolderOpenRedPacket.class);
//        } else {
...
//        }
    ...
    private static void setSessionListener() {
        SessionEventListener listener = new SessionEventListener() {
            ...
            public void onAvatarClicked(Context context, IMMessage message) {
                ...
                Intent intent = new Intent(context, userInfoActivity);
                if (message.getSessionType() == SessionTypeEnum.Team) {
                    intent.putExtra("group_id", message.getSessionId());
                }
                intent.putExtra("yx_id", message.getFromAccount());
            ...
            public void onAvatarLongClicked(Context context, IMMessage message) {
                ...
                if (message.getSessionType() == SessionTypeEnum.Team) {
                    if (NimUIKit.insertAitMember != null)
                        NimUIKit.insertAitMember.OnClick(message.getFromAccount(), message.getFromNick());
                }
            }
        ...
        NimUIKit.setMsgRevokeFilter(new MsgRevokeFilter() {
            ...
            public boolean shouldIgnore(IMMessage message) {
                if (message.getAttachment() != null && (message.getAttachment() instanceof AVChatAttachment ||
                        message.getAttachment() instanceof RTSAttachment ||
                        message.getAttachment() instanceof RedPacketAttachment) ||
                        MsgViewHolderFactory.getViewHolderByType(message) == ADViewHolder.class) {
    ...
    private static Class<? extends Activity> userInfoActivity;

    public static void setUserInfoActivity(Class<? extends Activity> clazz) {
        SessionHelper.userInfoActivity = clazz;
    }
}
