...
public class MessageListPanelEx {
    ...
    public static final int REQUEST_CODE_FORWARD_ALL = 0x03;
...
//            // resend的的情况，可能时间已经变化了，这里要重新检查是否要显示时间
    ...
    private class MessageLoader implements BaseFetchLoadAdapter.RequestLoadMoreListener, BaseFetchLoadAdapter.RequestFetchMoreListener {
        ...
        private void onMessageLoaded(final List<IMMessage> messages) {
            ...
            if(messages.size()>0) {
               // System.out.println("-----   没有msgType ？？？messages=" + messages.get(messages.size()-1).getRemoteExtension());
                if(messages.get(messages.size()-1).getRemoteExtension()!=null&&messages.get(messages.size()-1).getRemoteExtension().toString().contains("msgType")){
                    isService=true;
                }
            }
    ...
    private class MsgItemEventListener implements MsgAdapter.ViewHolderEventListener {
        ...
        private void prepareDialogItems(final IMMessage selectedItem, CustomAlertDialog alertDialog) {
         ...
         //   System.out.println("----------长按消息-prepareDialogItems");
            MessageAudioControl.getInstance(container.activity).stopAudio();
            if (selectedItem.getDirect() == MsgDirectionEnum.In) {
             //   System.out.println("---------3-长按消息-prepareDialogItems");
                if (selectedItem.getAttachment() != null && selectedItem.getAttachment().toJson(false) != null) {
                    String msgemoji = selectedItem.getAttachment().toJson(false);
                  //  System.out.println("-----------mMsgAttachment.toJson"+msgemoji);
                    if (msgemoji.contains("emoji_link")) {
                        //长按表情，显示添加表情
                        longClickItemSticker(selectedItem, alertDialog);
                    }
                }
            }
            // 0 EarPhoneMode
//            longClickItemEarPhoneMode(alertDialog, msgType);
            ...
            if (!NimUIKitImpl.getMsgForwardFilter().shouldIgnore(selectedItem) && !recordOnly) {
                longClickItemForwardToSession(selectedItem, alertDialog);
              //  if (selectedItem.getMsgType() == MsgTypeEnum.image || selectedItem.getMsgType() == MsgTypeEnum.text || selectedItem.getMsgType() == MsgTypeEnum.audio || selectedItem.getMsgType() == MsgTypeEnum.video || selectedItem.getMsgType() == MsgTypeEnum.location)
                if (selectedItem.getMsgType() == MsgTypeEnum.image || selectedItem.getMsgType() == MsgTypeEnum.text ) {
                    longClickItemCollection(selectedItem, alertDialog);
                }
//                // 6 forward to person
...
//                longClickItemForwardToTeam(selectedItem, alertDialog);
        ...
        private boolean enableRevokeButton(IMMessage selectedItem) {
            if ((selectedItem.getStatus() == MsgStatusEnum.success
                    || selectedItem.getStatus() == MsgStatusEnum.read
                    || selectedItem.getStatus() == MsgStatusEnum.unread)
        ...
        // 长按菜表情--添加表情
        private void longClickItemSticker(final IMMessage item, CustomAlertDialog alertDialog) {
            //StickerAttachment
            alertDialog.addItem("添加表情", new CustomAlertDialog.onSeparateItemClickListener() {
                ...
                public void onClick() {
                    ...
                    LocalBroadcastManager.getInstance(container.activity).sendBroadcast(intent);
                }
            });
        }
        ...
        // 长按菜单项 -- 转发到最近聊天
        private void longClickItemCollection(final IMMessage item, CustomAlertDialog alertDialog) {
            alertDialog.addItem("收藏", new CustomAlertDialog.onSeparateItemClickListener() {
                ...
                public void onClick() {
                    ...
                    intent.putExtra("mess", item);
                    LocalBroadcastManager.getInstance(container.activity).sendBroadcast(intent);
                    // SessionActivity.startActivityForResult(container.activity, REQUEST_CODE_FORWARD_ALL);
        ...
        // 长按菜单项 -- 转发到最近聊天
        private void longClickItemForwardToSession(final IMMessage item, CustomAlertDialog alertDialog) {
            alertDialog.addItem("转发", new CustomAlertDialog.onSeparateItemClickListener() {
...
//                    NIMClient.getService(MsgService.class).revokeMessage(item).setCallback(new RequestCallback<Void>() {
    ...
    // 删除消息
    private void addStickerItem(IMMessage messageItem, boolean isRelocateTime) {
        List<IMMessage> messages = new ArrayList<>();
        for (IMMessage message : items) {
            if (message.getUuid().equals(messageItem.getUuid())) {
                continue;
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case REQUEST_CODE_FORWARD_ALL:
                ArrayList<String> types = data.getStringArrayListExtra(SessionActivity.RESULT_TYPE);
            ...
            case REQUEST_CODE_FORWARD_TEAM:
                for (String session : selected) {
                    doForwardMessage(session, SessionTypeEnum.Team);
                }
                break;
            case REQUEST_CODE_FORWARD_PERSON:
                for (String session : selected) {
                    doForwardMessage(session, SessionTypeEnum.P2P);
                }
                break;
        }
