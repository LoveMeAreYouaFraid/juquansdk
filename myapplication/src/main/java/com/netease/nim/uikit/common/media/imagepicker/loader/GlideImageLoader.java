...
public class GlideImageLoader implements ImageLoader {
    ...
    @Override
    public void displayImage(Context context, String path, ImageView imageView, int width, int height, final LoadListener listener) {
        RequestOptions options = new RequestOptions()
                .error(0)           //设置错误图片
    ...
    @Override
    public void downloadImage(Context context, String path, final LoadListener listener) {
        RequestOptions options = new RequestOptions()
                .error(0)           //设置错误图片
                .placeholder(0)     //设置占位图片
                .diskCacheStrategy(DiskCacheStrategy.ALL)      //缓存全尺寸
