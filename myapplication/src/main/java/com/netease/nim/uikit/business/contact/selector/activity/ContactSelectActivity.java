...
public class ContactSelectActivity extends UI implements View.OnClickListener, SearchView.OnQueryTextListener {
    ...
    public static final String RESULT_DATA_VIP = "RESULT_DATA_VIP"; // 返回结果存在Vip群
    // adapter
    ...
    public static class Option implements Serializable {
         ...
         * vipTeam
         */
        public boolean isVipTeam = false;
    ...
    public static void startActivityForResultFragment(Context context,Fragment mFragment, Option option, int requestCode) {
        Intent intent = new Intent();
        intent.putExtra(EXTRA_DATA, option);
        intent.setClass(context, ContactSelectActivity.class);
    ...
    private void initAdapter() {
        ...
        if (option.type == ContactSelectType.TEAM_MEMBER && !TextUtils.isEmpty(this.option.teamId)) {
            ...
            if(option.isVipTeam){
                dataProvider = new ContactDataProvider(option.isVipTeam,ItemTypes.TEAM);
            }else{
                dataProvider = new ContactDataProvider(ItemTypes.TEAM);
            }
        } else {
    ...
    public void onSelectedVip(ArrayList<String> selects) {
        Intent intent = new Intent();
        intent.putStringArrayListExtra(RESULT_DATA, selects);
        intent.putExtra(RESULT_DATA_VIP, 1);
        setResult(Activity.RESULT_OK, intent);
        this.finish();
    }
