...
import com.netease.nim.uikit.common.ui.widget.LazyGridView;
...
public class AdvancedTeamMemberActivity extends UI implements TAdapterDelegate,
        TeamMemberAdapter.RemoveMemberCallback, TeamMemberAdapter.AddMemberCallback, TeamMemberHolder.TeamMemberHolderEventListener, OnLoadMoreListener {
    ...
    public void onLoadMore(@NonNull RefreshLayout refreshLayout) {
        //每次滚动到底部就在显示一部分
        indextemp++;
        if(getPageData(indextemp, pageSize)!=null) {
            for (String account : getPageData(indextemp, pageSize)) {
    ...
    public class MyRunable implements  Runnable{
      private  List<TeamMember> members;
        MyRunable(List<TeamMember> members){
    ...
    private void findViews() {
        mSwipeLayout= (SmartRefreshLayout) findViewById(R.id.refreshLayout);
      ...
      // TeamInfoGridView teamInfoGridView = (TeamInfoGridView) findViewById(R.id.team_member_grid);
        LazyGridView teamInfoGridView = (LazyGridView) findViewById(R.id.team_member_grid);
    ...
    private void updateTeamMember(final List<TeamMember> members) {
      ...
      //  addTeamMembers(members, true);
        new Thread(new MyRunable(members)).start();
    }
     ...
     * 分页加载索引器
     *
     * @param index    注意这个过些从0开始
     * @param pageSize
     * @return
     */
    public List<String> getPageData(int index, int pageSize) {

        if (memberAccounts==null||memberAccounts.size() == 0 || memberAccounts.size() <= pageSize) return null;

        int nTotalPage = memberAccounts.size() / pageSize;
        int nRestNum = memberAccounts.size() % pageSize;

        if (index < nTotalPage) {

            int max = -1;
            if((index + 1) * pageSize >= memberAccounts.size()){
                max = memberAccounts.size();
            }else{
                max = (index + 1) * pageSize;
            }
            return memberAccounts.subList(index * pageSize, max);

        } else if (index == nTotalPage) {

            if (nRestNum == 0) {
                return memberAccounts.subList((index - 1) * pageSize, index * pageSize);
            } else {
                return memberAccounts.subList(index * pageSize, index * pageSize + nRestNum);
            }

        } else {
            System.out.println("输入页面索引非法，列表最大页数为：" + nTotalPage + "  输入的页面数为：" + index);
            return null;
        }
    }
    ...
    public void onHeadImageViewClick(String account) {
        if(isSelfAdmin||isSelfManager) {
            ...
            userProfileEvent.action(AdvancedTeamMemberActivity.this, account);
        }
    }
    ...
    public static void setUserProfileEventEvent(EventInterface.UserProfileEvent userProfileEvent) {
        AdvancedTeamMemberActivity.userProfileEvent = userProfileEvent;
    }
    @Override
