...
public class MsgViewHolderFactory {
    ...
    public static Class<? extends MsgViewHolderBase> getViewHolderByType(IMMessage message) {
        if (message.getMsgType() == MsgTypeEnum.text) {
            ...
            try {
                JSONObject data = MsgViewHolderCobraBase.getMessageCustomContent(message);
                if (data != null) {
                    int type = data.getInt("type");
                    if (type == 10) { //图文广告
                        return ADViewHolder.class;
                    } else if (type == 11) {
                        return ADRPHolder.class;
                    }
                }
            } catch (Exception e) {
                Log.e("MsgViewHolderBase", e.getMessage());
            }
    ...
    public static List<Class<? extends MsgViewHolderBase>> getAllViewHolders() {
        ...
        list.add(ADViewHolder.class);
        list.add(ADRPHolder.class);
