...
public class NimUIKit {
    ...
    public static void startP2PSession2(Context context, String account, IMMessage anchor,boolean isService) {
        NimUIKitImpl.startP2PSession2(context, account, anchor,isService);
    }

    /**
     * 打开社群界面，若开发者未设置 {@link NimUIKitImpl#setCommonTeamSessionCustomization(SessionCustomization)},
    ...
    public static void startChatting2(Context context, String id, SessionTypeEnum sessionType, SessionCustomization
            customization, IMMessage anchor,boolean isService) {
        NimUIKitImpl.startChatting2(context, id, sessionType, customization, anchor,isService);
    }

    /**
     * 打开一个聊天窗口（用于从聊天信息中创建社群时，打开社群）
     *
     * @param context       上下文
     * @param id            聊天对象ID（用户帐号account或者群组ID）
     * @param sessionType   会话类型
     ...
     * @param requestCode startActivityForResult使用的请求码
     */
    public static void startContactSelectorFragment(Context context, Fragment mFragment , ContactSelectActivity.Option option, int requestCode) {
        NimUIKitImpl.startContactSelectorFragment(context,mFragment, option, requestCode);
    }
    ...
    public static AitManager.InsertAitMember insertAitMember;

    public static void setInsertAitMember(AitManager.InsertAitMember ia) {
        insertAitMember = ia;
    }
}
